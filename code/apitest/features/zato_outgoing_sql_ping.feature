...
Feature: zato.outgoing.sql.ping
  ...
  @outgoing.sql.ping
    ...
    Given JSON Pointer "/engine" in request is "sqlite"
    Given JSON Pointer "/host" in request is "localhost"
    Given JSON Pointer "/port" in request is "5432"
    Given JSON Pointer "/db_name" in request is "apitest.sqlite3"
  ...
  @outgoing.sql.ping
  Scenario: Ping created SQL connection

    Given address "$ZATO_API_TEST_SERVER"
    Given Basic Auth "$ZATO_API_TEST_PUBAPI_USER" "$ZATO_API_TEST_PUBAPI_PASSWORD"

    Given URL path "/zato/json/zato.outgoing.sql.ping"
    Given format "JSON"
    Given request is "{}"
