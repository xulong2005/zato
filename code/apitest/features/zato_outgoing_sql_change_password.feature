...
Feature: zato.outgoing.sql.create
  ...
  @outgoing.sql.change-password
  Scenario: Change password for created SQL connection

    Given address "$ZATO_API_TEST_SERVER"
    Given Basic Auth "$ZATO_API_TEST_PUBAPI_USER" "$ZATO_API_TEST_PUBAPI_PASSWORD"

    Given URL path "/zato/json/zato.outgoing.sql.change-password"
    Given format "JSON"
    Given request is "{}"
    Given JSON Pointer "/id" in request is "#def_id"
    Given JSON Pointer "/password1" in request is "taco1"
    Given JSON Pointer "/password2" in request is "taco1"
