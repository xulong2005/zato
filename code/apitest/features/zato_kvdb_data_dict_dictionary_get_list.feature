...
Feature: kvdb.data.dict.dictionary.get-list
  ...
  @kvdb.data-dict.dictionary.get-list
  Scenario: Invoke get-list to get the at least the 2 systems we just defined

    Given address "$ZATO_API_TEST_SERVER"
    Given Basic Auth "$ZATO_API_TEST_PUBAPI_USER" "$ZATO_API_TEST_PUBAPI_PASSWORD"

    Given URL path "/zato/json/zato.kvdb.data-dict.dictionary.get-list"

    When the URL is invoked

    Then JSON Pointer "/zato_env/result" is "ZATO_OK"
    And JSON Pointer "/zato_kvdb_data_dict_dictionary_get_list_response" isn't an empty list
