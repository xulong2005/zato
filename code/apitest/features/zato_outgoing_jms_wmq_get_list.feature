...
Feature: zato.outgoing.jms-wmq.get-list
  ...
  @outgoing.jms-wmq.get-list
  Scenario: Get connection list

      Given address "$ZATO_API_TEST_SERVER"
      Given Basic Auth "$ZATO_API_TEST_PUBAPI_USER" "$ZATO_API_TEST_PUBAPI_PASSWORD"

      Given URL path "/zato/json/zato.outgoing.jms-wmq.get-list"
      ...
      Then status is "200"
      And JSON Pointer "/zato_outgoing_jms_wmq_get_list_response" isn't an empty list
      And I sleep for "1"
