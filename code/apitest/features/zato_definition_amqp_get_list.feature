...
Feature: zato.definition.amqp.get-list
  ...
  @definition.amqp.get-list
  Scenario: Get amqp definition list
    Given address "$ZATO_API_TEST_SERVER"
    Given Basic Auth "$ZATO_API_TEST_PUBAPI_USER" "$ZATO_API_TEST_PUBAPI_PASSWORD"

    Given URL path "/zato/json/zato.definition.amqp.get-list"
    ...
    Then status is "200"
    And JSON Pointer "/zato_env/result" is "ZATO_OK"
    And JSON Pointer "/zato_definition_amqp_get_list_response" isn't an empty list
