...
Feature: zato.outgoing.sql.get-list
  ...
  @outgoing.sql.get-list
    Scenario: Get SQL Connection List
    Given address "$ZATO_API_TEST_SERVER"
    Given Basic Auth "$ZATO_API_TEST_PUBAPI_USER" "$ZATO_API_TEST_PUBAPI_PASSWORD"

    Given URL path "/zato/json/zato.outgoing.sql.get-list"
    ...
    And JSON Pointer "/zato_outgoing_sql_get_list_response" isn't an empty list
