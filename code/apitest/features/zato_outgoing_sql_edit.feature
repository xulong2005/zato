...
Feature: zato.outgoing.sql.edit
  ...
  @outgoing.sql.edit
    Scenario: Edit SQL Connection
    Given address "$ZATO_API_TEST_SERVER"
    Given Basic Auth "$ZATO_API_TEST_PUBAPI_USER" "$ZATO_API_TEST_PUBAPI_PASSWORD"

    Given URL path "/zato/json/zato.outgoing.sql.edit"
    ...
    Given JSON Pointer "/engine" in request is "mysql"
    Given JSON Pointer "/host" in request is "localhost"
    Given JSON Pointer "/port" in request is "3306"
    Given JSON Pointer "/db_name" in request is "true"
    Given JSON Pointer "/username" in request is "zato_db"
    Given JSON Pointer "/pool_size" in request is "10"
    ...
    And I store "/zato_outgoing_sql_edit_response/id" from response under "def_id"
    And JSON Pointer "/zato_outgoing_sql_edit_response/id" is any integer
    And I sleep for "2"
